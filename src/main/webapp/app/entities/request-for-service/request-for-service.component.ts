...
export default class RequestForService extends mixins(AlertMixin) {
  ...
  public sendToRespond(instance: IRequestForService): void {          
      this.$router.push({ name: 'RequestRespondCreateChoice', params: { requestForServiceId: JSON.stringify(instance.id).replace(/["]/g, '')  } })
  }
  ...
  public previewRespond(instance: IRequestForService): void {   
    this.requestRespondService()
      ...
      .then(res => {
          if(res.id){
            this.$router.push({ name: 'RequestRespondViewForService', params: { requestForServiceId: JSON.stringify(instance.id).replace(/["]/g, '') } });
          }
        })
        .catch(() => {
